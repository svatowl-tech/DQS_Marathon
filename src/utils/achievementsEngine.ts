...
export function calculateAchievements(
  ...
  const totalApples = sortedLogs.reduce((acc, l) => acc + (l.trackers?.apples || 0), 0);
  const totalWatermelons = sortedLogs.reduce((acc, l) => acc + (l.trackers?.watermelons || 0), 0);
  const totalMelons = sortedLogs.reduce((acc, l) => acc + (l.trackers?.melons || 0), 0);
  const totalVegetables = sortedLogs.reduce((acc, l) => acc + (l.servings?.vegetables || 0) + (l.servings?.greens || 0), 0);

  const permanentNutrition: AchievementItem[] = [
    {
      id: 'veg_first',
      title: 'Первый урожай',
      description: 'Съедена первая порция дачных овощей или зелени',
      icon: '🥒',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalVegetables >= 1,
      progressText: `${Math.min(totalVegetables, 1)} / 1 порц.`,
      badgeColor: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
    },
    {
      id: 'veg_25',
      title: 'Дачный эксперт',
      description: '25 порций овощей. Зайцы смотрят на вас с уважением',
      icon: '🥕',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalVegetables >= 25,
      progressText: `${Math.min(totalVegetables, 25)} / 25 порц.`,
      badgeColor: 'bg-orange-500/15 text-orange-400 border-orange-500/30',
    },
    {
      id: 'veg_100',
      title: 'Помидорный магнат',
      description: '100 порций овощей! Вы официально спонсор летнего салата',
      icon: '🍅',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalVegetables >= 100,
      progressText: `${Math.min(totalVegetables, 100)} / 100 порц.`,
      badgeColor: 'bg-rose-500/15 text-rose-400 border-rose-500/30',
    },
    {
      id: 'veg_300',
      title: 'Гроза теплиц',
      description: '300 порций! Ваша кровь на 80% состоит из огуречного сока',
      icon: '👑',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalVegetables >= 300,
      progressText: `${Math.min(totalVegetables, 300)} / 300 порц.`,
      badgeColor: 'bg-emerald-600/20 text-emerald-300 border-emerald-500/40',
    },
    {
      id: 'watermelon_1',
      title: 'Первый арбуз',
      description: 'Открыт сезон сахарных арбузов!',
      icon: '🍉',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalWatermelons >= 1,
      progressText: `${Math.min(totalWatermelons, 1)} / 1 порц.`,
      badgeColor: 'bg-pink-500/15 text-pink-400 border-pink-500/30',
    },
    {
      id: 'watermelon_5',
      title: 'Арбузный сомелье',
      description: 'Постучали по 5 арбузам и съели их',
      icon: '🍉',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalWatermelons >= 5,
      progressText: `${Math.min(totalWatermelons, 5)} / 5 порц.`,
      badgeColor: 'bg-rose-500/15 text-rose-400 border-rose-500/30',
    },
    {
      id: 'watermelon_20',
      title: 'Сахарный гигант',
      description: '20 порций арбуза. Главное — не лопнуть!',
      icon: '🍉',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalWatermelons >= 20,
      progressText: `${Math.min(totalWatermelons, 20)} / 20 порц.`,
      badgeColor: 'bg-fuchsia-500/15 text-fuchsia-400 border-fuchsia-500/30',
    },
    {
      id: 'watermelon_50',
      title: 'Повелитель бахчи',
      description: '50 порций арбуза. Вы состоите из воды и сахара на 99%!',
      icon: '👑',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalWatermelons >= 50,
      progressText: `${Math.min(totalWatermelons, 50)} / 50 порц.`,
      badgeColor: 'bg-pink-600/20 text-pink-300 border-pink-500/40',
    },
    {
      id: 'melon_1',
      title: 'Первая дынька',
      description: 'Медовая торпеда пошла в дело',
      icon: '🍈',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalMelons >= 1,
      progressText: `${Math.min(totalMelons, 1)} / 1 порц.`,
      badgeColor: 'bg-yellow-500/15 text-yellow-400 border-yellow-500/30',
    },
    {
      id: 'melon_5',
      title: 'Дынный гурман',
      description: '5 порций сладчайшей дыни съедено',
      icon: '🍈',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalMelons >= 5,
      progressText: `${Math.min(totalMelons, 5)} / 5 порц.`,
      badgeColor: 'bg-amber-500/15 text-amber-400 border-amber-500/30',
    },
    {
      id: 'melon_20',
      title: 'Медовая душа',
      description: '20 порций! Восток — дело тонкое и сладкое',
      icon: '🍈',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalMelons >= 20,
      progressText: `${Math.min(totalMelons, 20)} / 20 порц.`,
      badgeColor: 'bg-orange-500/15 text-orange-400 border-orange-500/30',
    },
    {
      id: 'apple_debut',
      title: 'Яблочный дебют',
      description: 'Открыт сезон яблок! Съедено первое яблоко',
      icon: '🍎',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalApples >= 1,
      progressText: `${Math.min(totalApples, 1)} / 1 шт.`,
      badgeColor: 'bg-red-500/15 text-red-400 border-red-500/30',
    },
    {
      id: 'apple_lover',
      title: 'Любитель яблок',
      description: 'Съедено 10 яблок в этом сезоне',
      icon: '🍏',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalApples >= 10,
      progressText: `${Math.min(totalApples, 10)} / 10 шт.`,
      badgeColor: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
    },
    {
      id: 'apple_fan',
      title: 'Яблочная душа',
      description: 'Съедено 50 яблок. Доктор забыл дорогу к вашему дому!',
      icon: '🥧',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalApples >= 50,
      progressText: `${Math.min(totalApples, 50)} / 50 шт.`,
      badgeColor: 'bg-orange-500/15 text-orange-400 border-orange-500/30',
    },
    {
      id: 'apple_king',
      title: 'Яблочный магнат',
      description: 'Съедено 100 яблок! Вы почетный спонсор яблочного спаса.',
      icon: '🌳',
      category: 'nutrition',
      type: 'permanent',
      unlocked: totalApples >= 100,
      progressText: `${Math.min(totalApples, 100)} / 100 шт.`,
      badgeColor: 'bg-rose-500/15 text-rose-400 border-rose-500/30',
    },
    {
      id: 'nut_first_log',

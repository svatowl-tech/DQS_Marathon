...
function transformServings(servingsObj, title, description) {
  ...
  for (let [key, val] of Object.entries(servingsObj)) {
    val = parseFloat(val);
    ...
    if (key === 'nuts_seeds') targetKey = 'nuts';
      ...
      if (/пиво|вино|коньяк|водка|виски|ром|джин|сидр|коктейль|шампанск|ликёр|вермут|эль|але|портер/.test(text)) {
        targetKey = 'alcohol';
      } else {
...
files.forEach(file => {
  ...
  // Match each object inside export const ... = [ ... ];
  // We can process item by item by replacing `servings: { ... }` blocks
  const updatedContent = content.replace(/\{\s*id:[\s\S]*?\n\s*\}/g, (itemBlock) => {
    // Extract title
    ...
    // Extract servings match
    const servingsMatch = itemBlock.match(/servings:\s*\{([^}]*)\}/);
    if (!servingsMatch) return itemBlock;

    const oldServingsStr = servingsMatch[1];
    const oldServings = {};
    oldServingsStr.split(',').forEach(p => {
      const parts = p.split(':');
      if (parts.length === 2) {
    ...
    return itemBlock.replace(`servings: {${oldServingsStr}}`, `servings: { ${newServingsFormatted} }`);
  });
